...
def imports(request):
   if 'file' not in request.FILES or 'tpid' not in request.POST or not request.POST['tpid']:
      return redirect('/static/app/index.html#/import/%s' % quote_plus(b64encode(bytes('Invalid data', 'UTF-8'))))
   ...
   return redirect('/static/app/index.html#/import/%s' % quote_plus(b64encode(bytes(response, 'UTF-8'))))
